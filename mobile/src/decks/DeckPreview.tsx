 ...
 *
 * A full-screen overlay rather than a Modal: it is opened from inside the deck
 * picker's modal, and stacking native modals — then dismissing both at once
 * when a deck is chosen — is what locks iOS up.
 */
export default function DeckPreview(props: {
    ...
    // A zoom must never outlive the deck it was opened from.
    useEffect(() => {
        if (!deck) {
    ...
    return (
        <View style={[StyleSheet.absoluteFill, styles.container, { paddingTop: insets.top + 12 }]}>
            <View style={styles.header}>
                <View style={{ flex: 1 }}>
                    ...
                    <View style={styles.metaRow}>
                        ...
                        {sas !== undefined ? <Text style={styles.sas}>{sas} SAS</Text> : null}
                        {set ? <Text style={styles.set}>{set}</Text> : null}
                    </View>
                </View>
                <Button small variant='secondary' title='Back' onPress={props.onClose} />
            </View>
