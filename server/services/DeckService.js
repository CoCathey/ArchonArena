...
/**
 * ARCHON (N19/F9): a deck's ARI in SQL - the stored index when the rating
 * engine has ever touched the deck, otherwise the SAS/AERC seed it would
 * have started at. The exact rule `AriService.effectiveAri` applies in
 * JavaScript, written once here so a query that FILTERS on ARI and a reader
 * that DISPLAYS it cannot disagree about a deck's number.
 *
 * Expects `ds` (DeckSas) and `da` (DeckAri) joined on the deck's uuid.
 */
const EFFECTIVE_ARI_SQL =
    'COALESCE(da."Ari", CASE ' +
    'WHEN ds."SasRating" IS NOT NULL AND ds."AercScore" IS NOT NULL ' +
    'THEN (ds."SasRating" + ds."AercScore") / 2.0 ' +
    'ELSE COALESCE(ds."SasRating", ds."AercScore") END)';
