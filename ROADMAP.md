...
> New systems are built *around* the gameplay engine as loosely-coupled services, never by
...
## Phase 0 — Working fork & foundation  *(PRIORITY)*
...
- [ ] Get server + client building locally (`npm install`, dev build passes).
- [ ] Get test suite running; record baseline pass rate before any changes.
- [ ] CI pipeline (GitHub Actions): lint, test, build on every PR.
- [ ] Dockerfile + docker-compose for one-command local stack (node server, client,
      PostgreSQL, Redis, MongoDB-compat layer if needed by legacy code).
...
## Phase 1 — Rebrand to Archon Arena  *(PRIORITY)*
...
## Phase 2 — Production deployment on ArchonArena.com  *(PRIORITY)*

- [ ] Choose hosting (VPS w/ Docker Compose to start; K8s later if needed) — decision doc.
- [ ] Production docker-compose: web, game node(s), Postgres, Redis, reverse proxy.
- [ ] Nginx/Caddy reverse proxy with automatic TLS (Let's Encrypt).
- [ ] Point ArchonArena.com DNS (Porkbun) at the host; document DNS records needed
      (A/AAAA for apex + www, optional CNAME for cdn/assets).
- [ ] WebSocket pass-through for game server (socket.io) verified through proxy.
- [ ] Environment/secrets management (.env conventions, no secrets in repo).
...
## Phase 3 — Authentication: Keybringer SSO  *(PRIORITY)*
...
## Phase 4 — Deck service: Decks of KeyForge SAS integration  *(PRIORITY)*
...
## Phase 5 — Rating engine: SAS-adjusted Elo  *(PRIORITY)*
...
- [ ] Standalone **Rating Service** (pure functions + persistence adapter; no gameplay
      engine coupling).
- [ ] Core algorithm:
      - Expected score `E = 1 / (1 + 10^((Ro - Rp + sasWeight·ΔSAS)/400))` — ΔSAS shifts
        expectation toward the stronger deck.
      - Margin-of-victory multiplier from key differential (3–0, 3–1, 3–2 keys, forge-out
        vs. timeout, concession) similar to FIDE/Glicko margin variants.
      - `R' = R + K · movMultiplier(keyDiff) · (S − E)`.
...
## Phase 6 — Rankings & leaderboards  *(PRIORITY)*
...
## Phase 7 — Tournament engine  *(PRIORITY)*
...
## Cross-cutting: Site administration  *(build alongside every phase)*
